    ...
    public class ProductTypeRepository
    {
        ...
        public dynamic GetSpecificProductTypeFullInfoBySpecialNameAndYear (string productTypeName, int year, ProductTypeOperations productTypeOperations)
        {
            ...
            using (var dbConnection = new SQLiteConnection(AppConfiguration.ConnectionString))
            {
                ...
                string getExistingServiceQuery =
                    $@"SELECT * FROM ProductType PT
                       WHERE PT.{productType} = '{productTypeName}' AND PT.IdByInvoiceNumberYearCreation = {year} AND PT.{productType} IS NOT NULL AND PT.{productTypeQuantity} IS NOT NULL AND 
                             PT.{productTypePrice} IS NOT NULL
                       ORDER BY PT.IdByInvoiceNumber DESC, PT.IdByInvoiceNumberYearCreation DESC
                    ";
